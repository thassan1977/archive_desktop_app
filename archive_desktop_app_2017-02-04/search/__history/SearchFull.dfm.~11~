inherited FSearchFull: TFSearchFull
  ...
  inherited JQ: TAmjSet
    CommandText = 
      ...
      'OJID) mParents,'#13#10'(select Z.ZTMPLTID from  JDMS.JPROJ_DOCS Z wher' +
      'e Z.ID = X.DOCID) DOCTMPLID'#13#10' FROM'#13#10'(SELECT ID, MTNAME, MTEXT, M' +
      'ASTERID, (case when MASTERID is null then ID'#13#10'    else MasterID ' +
      'end) DOCID'#13#10'FROM JDMS.FULLSEARCH_MASTER) X left join JDMS.JPROJ_' +
      'DOCS Y on X.DOCID = Y.ID'#13#10
    ...
    SQL.Strings = (
        ...
        '(select Z.ZTMPLTID from  JDMS.JPROJ_DOCS Z where Z.ID = X.DOCID)' +
        ' DOCTMPLID'
      ' FROM'
