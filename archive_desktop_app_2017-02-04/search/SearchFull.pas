...
type
  TFSearchFull = class(TJPmain)
    ...
    procedure JRCancelcloseExecute(Sender: TObject);ations }
       ...
       function getMetaDataID(pId, docId : string) : string;
    public
    { Public declarations }
...
function TFSearchFull.getMetaDataID(pId, docId: string) : string;
var
  metaDataID : String ;
begin
  metaDataID := '' ;
  JQDocMetaData.close;
  JQDocMetaData.params.paramByName('pid').AsString := docId;
  JQDocMetaData.Open ;
  if JQMetaIdSEt.RecordCount > 0 then
  begin
    if not jq.FieldByName('ZTMPLTID').isnull then
       metaDataID := JQDocMetaData.FieldByName('ZTMPLTID').AsString;
  end
  else
  begin
      JQMetaIdSEt.close;
...
begin
  ...
  if not jq.FieldByName('DOCTMPLID').isNull then
      metaID := jq.FieldByName('DOCTMPLID').asString
  else
  begin
      JQMetaIdSEt.close;
      ...
      JQMetaIdSEt.open;
      if JQMetaIdSEt.RecordCount > 0 then
          metaID := JQMetaIdSEt.FieldByName('metaid').AsString;
  ...
  begin
    try
      JQMetBLOB.close;
      JQMetBLOB.params.paramByName('pid').AsString := metaID;
      JQMetBLOB.open;
...
m_Sql :=  ' Select X.*, Y.PROJID, JDMS.ARCH_SEARCH.getAllProjectParents(Y.PROJID) mParents, '+
          ' (select Z.ZTMPLTID from  JDMS.JPROJ_DOCS Z where Z.ID = X.DOCID) DOCTMPLID ' +
          ' FROM '+
...
//   panel1.align := altop;
  ...
  procedure TFSearchFull.ProcessJsonNm( JsonForm :TTemplateForm;txt: String);
  var str,TableName:string;  // text to be modified
//  JsonForm : TTemplateForm;
  lab : Tlabel;
//  position_s,position_E :integer;
