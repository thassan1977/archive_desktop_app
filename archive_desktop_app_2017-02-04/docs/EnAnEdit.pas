...
public
    ...
    procedure SavePos( Canvas: TCanvas;var MarkList:Tlist);
    { MB Jan 08, 2003 }
...
begin
    ...
    {$Warnings Off}
    Result := StrToInt(String(ValueStr));
end;
...
begin
    ...
    {$Warnings Off}
    Font.Name  := String(ReadString( Str, Index ));
...
procedure ReadStickNote( const Mark    : TStickNoteMark;
                         const Str     : AnsiString;
                         var   Index   : Integer );
begin //ReadStickNote( TStickNoteMark(Mark), Str, StrIndex );
...
begin
    ...
    begin
        ...
        if Mark is TStickNoteMark then
            WriteStickNote( TStickNoteMark(Mark), Result ) //1
        else if Mark is TRectangleMark then
            WriteRectangle( TRectangleMark(Mark), Result ) //2
        else if Mark is TLineMark then
            WriteLine( TLineMark(Mark), Result ) //3
        else if Mark is TTextMark then  //4
...
procedure TGenericMark. SetID ( Value: INTEGER);
begin
    if Value = FID then
        Exit;
    FID := Value;
...
procedure TAnnotationsEditor.SavePos( Canvas: TCanvas;var MarkList:Tlist);
var
    i: integer;
...
begin
    for i := 0 to FMarks.Count - 1 do
        {$Warnings Off}
    //  TGenericMark( FMarks[i]).DrawMark( Canvas, TGenericMark( FMarks[i]).Left, TGenericMark( FMarks[i]).Top);
    MarkList.add(FMarks[i]);
   //   MarkList.add(FMarkstype[i]);
        {$Warnings On}
end;
