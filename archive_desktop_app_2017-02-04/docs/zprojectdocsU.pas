...
type
  Tzprojectdocsf = class(TJPmain)
    ...
    FolderUpdateFile: TAmjAction;
    getBlob: TAmjSet;
    actnAddFolder: TAmjAction;
    copyFileToversions: TAmjSet;
    ...
    procedure qVersionsBeforeOpen(DataSet: TDataSet);
...
procedure NameAndWidthColumn(dbgrid:tjazdbgrid;fieldn,newname:string;widthn:integer);
    ...
    procedure actnAddFolderHandle(Sender: TObject; var IsFailed: Boolean;
      var ErrorMsg: string);
    procedure FolderUpdateFileHandle(Sender: TObject; var IsFailed: Boolean;
      var ErrorMsg: string);
    procedure JQFoldersAfterOpen(DataSet: TDataSet);
    procedure AmjAction2Handle(Sender: TObject; var IsFailed: Boolean;
      var ErrorMsg: string);
  private
...
whitepaper :boolean;
  ...
  { Public declarations }
    ...
    function SaveBlobToLocalFile(id, mSql : string) : string;
...
function Tzprojectdocsf.SaveBlobToLocalFile(id, mSql : string) : string;
var
    res : string ;
    aStream: TFileStream;
    aFileName : string ;
...
var
  mimeTyp, blobFileName, mSql: string ;
begin

  if JQFolders.isEmpty then
      exit ;
     ...
     showMessage('«·„·› ·« Ì„À· ’Ê—…...”Ì „ › ÕÂ ··«” ⁄—«÷ ›ﬁÿ...'+#13+'√Ì  ⁄œÌ·  ﬁÊ„ »Â ·‰ ÌÕ›Ÿ ›Ì ﬁ«⁄œ… «·»Ì«‰« ...');
          ...
          try
              ...
              begin
                   ...
                   mSql := ' SELECT  ID, BIN_ON_DB from JDMS.JPROJ_DOCS_FOLDERS WHERE ID=:pid ' ;
                   blobFileName := SaveBlobToLocalFile(jqFolders.FieldByName('ID').asstring, mSql);
...
procedure Tzprojectdocsf.actnAddFolderHandle(Sender: TObject;
  var IsFailed: Boolean; var ErrorMsg: string);
var
...
Begin
  ...
  AmjActionList3.sqlstr := qVersions.commandtext;
...
procedure Tzprojectdocsf.FolderUpdateFileHandle(Sender: TObject;
  var IsFailed: Boolean; var ErrorMsg: string);
...
begin
  inherited;
    ...
    if JQFolders.FieldByName('ISFOLDER').asInteger = 1 then
    begin
        showMessage('·« Ì„ﬂ‰ﬂ  »œÌ· „Õ ÊÏ „Ã·œ »«·ﬂ«„·... »· Ì„ﬂ‰  »œÌ· „Õ ÊÏ „·› Ê«Õœ ﬂ· „—…');
        exit;
    end;
   ...
   try
        ...
        begin
              ...
              // try copy current fiile content tp versions table ...
              copyFileToversions.close ;
              copyFileToversions.params.ParamByName('pid').asString := JQFolders.FieldByName('ID').asString ;
              copyFileToversions.params.ParamByName('puser').asString := 'test';
              copyFileToversions.execute;

              // now update the current File with new content ..

              S1 := ExtractFileExt(OpenDialog.filename);
              delete(s1,1,1); // remove the dot '.'
              s2 := GetMIMEType(OpenDialog.filename);
             jadoset.Connection :=  JQFolders.AmjServerDB;
              jadoset.CommandText := 'select * from jdms.JPROJ_DOCS_FOLDERS where id = ' + JQFolders.FieldByName('ID').asString ;
              jadoset.open;
              jadoset.edit;
              jadoset.FieldByName('FILE_EXT').asString := s1;
              jadoset.FieldByName('FILE_MIME').asString := s2;
              jadoset.FieldByName('NAME').asString := ExtractFileName(OpenDialog.filename);
...
procedure Tzprojectdocsf.JQFoldersAfterOpen(DataSet: TDataSet);
begin
  inherited;
qVersions.Close ;
qVersions.Open;
...
//CheckDirty;
    ...
    try
        ...
        begin
            ...
            myPrnt := 'null';
            if JQFolders.FieldByName('ISFOLDER').asInteger = 1 then
                 myPrnt := JQFolders.FieldByName('ID').asString;
...
procedure Tzprojectdocsf.qVersionsBeforeOpen(DataSet: TDataSet);
begin
  inherited;
  qVersions.Params.ParamByName('pid').asstring := jqFolders.fieldbyname('id').asstring;
end;

procedure Tzprojectdocsf.AmjAction2Handle(Sender: TObject;
  var IsFailed: Boolean; var ErrorMsg: string);
var
  mimeTyp, blobFileName, mSql: string ;
begin

  if qVersions.isEmpty then
      exit ;


  inherited;
  mimeTyp := LowerCase(qVersions.fieldByName('FILE_MIME').asString) ;
  if (mimeTyp = 'image/bmp') or (mimeTyp = 'image/gif') or (mimeTyp='image/jpeg')
  or (mimeTyp='image/tiff') or (mimeTyp='image/x-icon') or (mimeTyp='image/png')
  or (mimeTyp='image/x-ms-bmp') then
  begin
  ...
  begin
     showMessage('«·„·› ·« Ì„À· ’Ê—…...”Ì „ › ÕÂ ··«” ⁄—«÷ ›ﬁÿ...'+#13+'√Ì  ⁄œÌ·  ﬁÊ„ »Â ·‰ ÌÕ›Ÿ ›Ì ﬁ«⁄œ… «·»Ì«‰« ...');
          ...
          try
                   // save blob to local file system...
                   mSql := ' SELECT  ID, FDATA as BIN_ON_DB from JDMS.JPROJ_DOCS_FILES_VERSION WHERE ID=:pid ' ;
                   blobFileName := SaveBlobToLocalFile(qVersions.FieldByName('ID').asstring, mSql);
                   if blobFileName = '' then
                   begin
                       ShowMessage('·« Ì„ﬂ‰ › Õ «·„·› Õ«·Ì« «·—Ã«¡ «·„Õ«Ê·… ·«Õﬁ«...');
